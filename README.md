...
The `options` parameter is an object with the following keys. Either `dataUri` or `audioContext` must be specified, but not both.

* `mediaUrl`: Audio or video media URL
* `dataUri`: (optional) If requesting waveform data from a server, this shuold be an object containing `arraybuffer` and/or `json` values
