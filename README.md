...
The `options` parameter is an object with the following keys. Only one of `dataUri`, or `rawData`, or `webAudio` must be specified.
...
* `rawData`: (optional) If using local or previously requested waveform data, this should be an object containing `arraybuffer` and/or `json` values
