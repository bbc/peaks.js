...
define([
  ...
  function WaveformZoomView(waveformData, $container, options) {
    ...
    that.playheadPixel = that.data.at_time(options.audioElement.currentTime);
      ...
      if (event.targetNode &&
        ...
        if (event.type == "mousedown") {
          ...
          that.stage.on("mousemove", function (event) {
            ...
            //that.syncPlayhead(that.playheadPixel);
          });
    ...
    bootstrap.pubsub.on("zoomview_user_seek", function (time, frameIndex) {
    ...
    var nudgeFrame = function nudgeFrame(step){
      var time = options.audioElement.currentTime;
